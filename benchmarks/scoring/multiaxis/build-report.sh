...
- **Functional $func/35** — $(
    if (( $(echo "$func >= 35" | bc -l) )); then echo "all journeys + build/boot/lint/ts clean."
      ...
      if [[ "$lint_score" == "0" ]]; then
        if [[ "$h" == "oma" ]]; then
          bullets+="lint -5 (intentional: oma defers lint to pre-commit/pre-push by design — see caveat #6); "
        else
          bullets+="lint failed; "
        fi
      fi
